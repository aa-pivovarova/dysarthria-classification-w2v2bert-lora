...
def prepare_dataset(batch, processor=None):
    ...
    try:
        ...
        if len(waveform) < min_length_samples:
            ...
            return None
        max_length_samples = int(4 * sampling_rate)
        if len(waveform) > max_length_samples:
            # print(f"⚠️ Audio too long ({len(waveform) / sampling_rate:.2f}s): {batch.get('filename', 'unknown')}")
            return None
...
def load_torgo(root_dir, db_root_dir, output_csv):
...
def load_easycall(root_dir, db_root_dir, output_csv):
    if os.path.exists(output_csv):
        df = pd.read_csv(output_csv)
        print(f"Found CSV: {output_csv}")
    else:
        from pathlib import Path
        data = []
        root_path = Path(root_dir + db_root_dir)
        for wav_path in root_path.rglob("*.wav"):
...
def transform_to_hfdataset(train_df, val_df, test_df):
    def create_filtered_dataset(df, name="dataset"):
         ...
         def path_exists(example):
             #print("/usr/src/app/" + example["audio"])
             return os.path.exists("/usr/src/app/" + example["audio"])
